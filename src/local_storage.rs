...
impl MutStore<Key> for LocalStorage {
    fn generate_id(self, table_name: &str) -> MutResult<Self, Key> {
        let prefix = self.get_id_prefix(table_name);
        let table_name = table_name.to_string();

        let key = match get_item(&prefix).as_string() {
            Some(v) => {
                let Key { id, .. } = try_self!(self, serde_json::from_str(&v));
    ...
    fn insert_data(self, key: &Key, row: Row) -> MutResult<Self, Row> {
    ...
    fn delete_data(self, key: &Key) -> MutResult<Self, ()> {
        let prefix = self.get_data_prefix(&key.table_name);
...
impl Store<Key> for LocalStorage {
    ...
    fn scan_data(&self, table_name: &str) -> Result<RowIter<Key>> {
